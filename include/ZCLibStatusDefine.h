 ...
 *  @enum 智齿连接业务状态码
 */
typedef NS_ENUM(NSInteger, ZCStatusCode) {
    /**
     *  连接失败。
     */
     ZC_CONNECT_FAIL = -1,
...
typedef NS_ENUM(NSInteger, ZCConnectUserStatusCode) {
    ...
    /** * 人工服务连接失败，已重新连接。 */ ZCConnectUserServerFailed = -1,
    
    /** * 连接失败。 */ ZCConnectUserOfWaiting = 0,

    /** * 连接成功。 */ ZCConnectUserSuccess = 1,
    
    /** * 没有客服在线。 */ ZCConnectUserNoAdmin = 2,
    
    /** * 用户被拉黑 */ZCConnectUserBeBlock = 3,
    
    /** * 用户已经转人工无需再次请求 */ZCConnectUserBeConnected = 4,
